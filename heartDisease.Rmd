...
1. **Heart Disease Classification:** Develop and evaluate ML classification models to predict **Heart Disease (Yes/No)** using health indicators and lifestyle determinants.  

2. **BMI Regression:** Apply ML regression techniques to predict **BMI (continuous)** using relevant lifestyle and health-related features.  

3. **Determinant Analysis:** Quantitatively assess the importance of key socioeconomic and behavioral variables (e.g., education, income, smoking, physical activity, diet) for both tasks.  

4. **Actionable Insights:** Translate the most influential determinants into clear, prioritized, evidence-based recommendations for public health and clinical intervention.
...
lightGBM_conf <- table(Predicted = pred_factor, Actual = test_factor)

lightGBM_acc <- mean(pred == test_y_num)
lightGBM_prec <- caret::precision(pred_factor, test_factor)
lightGBM_rec <- caret::recall(pred_factor, test_factor)
lightGBM_f1 <- caret::F_meas(pred_factor, test_factor)
lightGBM_auc <- auc(test_y_num, pred_prob)

cat("\n================ FINAL LIGHTGBM PERFORMANCE ================\n")
print(lightGBM_conf)
cat(sprintf(
  "\nAccuracy: %.4f\nPrecision: %.4f\nRecall: %.4f\nF1 Score: %.4f\nAUC: %.4f\n",
  lightGBM_acc, lightGBM_prec, lightGBM_rec, lightGBM_f1, lightGBM_auc
))
```
...
This classification task compares LightGBM (SMOTE + Bayesian Optimization) and XGBoost (class weighting + Bayesian Optimization) for predicting heart disease. Performance is evaluated on the test set using Accuracy, Precision, Recall, F1-score, and AUC to determine which approach handles class imbalance better.
...
### 12A.1 Classification comparison

This table compares LightGBM and XGBoost on the heart disease classification task using test-set Accuracy, Precision, Recall, F1-score, and AUC.

```{r}
clf_compare <- tibble(
  Model = c("LightGBM (SMOTE + BO)", "XGBoost (Class Weight + BO)"),
  Accuracy  = c(lightGBM_acc, xgb_acc),
  Precision = c(lightGBM_prec, xgb_prec),
  Recall    = c(lightGBM_rec, xgb_rec),
  F1        = c(lightGBM_f1, xgb_f1),
  AUC       = c(as.numeric(lightGBM_auc), as.numeric(xgb_auc))
) %>%
...
### 12A.2 Classification comparison

This bar chart visualizes the classification metrics for both models to make performance differences easier to compare.

```{r}
clf_long <- clf_compare %>%
  pivot_longer(-Model, names_to = "Metric", values_to = "Value")

ggplot(clf_long, aes(x = Metric, y = Value, fill = Model)) +
...
#### Conclusion
Based on the test-set results, LightGBM (SMOTE + BO) achieves higher Accuracy, Recall, and F1-score, indicating stronger overall performance and better detection of heart disease cases. In contrast, XGBoost (Class Weight + BO) attains slightly higher Precision and AUC, suggesting better discrimination and fewer false positives. Overall, LightGBM is more suitable when prioritizing correct identification of heart disease cases, while XGBoost may be preferred when precision is more critical.

### 12B.1 Regression comparison

This table compares Ranger Random Forest and XGBoost on the BMI regression task using test-set RMSE, MAE, MAPE, R², and Adjusted R².

```{r}
reg_compare <- tibble(
  Model = c("Random Forest (Ranger + BO)", "XGBoost (Random Search + CV)"),
  RMSE   = c(rf_rmse, xgb_rmse),
  MAE    = c(rf_mae,  xgb_mae),
  MAPE   = c(rf_mape, xgb_mape),
  R2     = c(rf_r2,   xgb_r2),
  Adj_R2 = c(rf_adj_r2, xgb_adj_r2)
) %>%
  mutate(across(-Model, ~ round(.x, 4)))
...
### 12B.2 Regression comparison

This bar chart visualizes the regression metrics for both models to clearly show which model performs better across each metric.

```{r}
reg_long <- reg_compare %>%
  pivot_longer(-Model, names_to = "Metric", values_to = "Value")

ggplot(reg_long, aes(x = Metric, y = Value, fill = Model)) +
  geom_col(position = "dodge") +
  labs(
    title = "Regression Model Comparison (Test Set)",
    x = "Metric", y = "Value"
  ) +
  theme_minimal(base_size = 13)
```
...
#### Conclusion
For the BMI regression task, Random Forest (Ranger + BO) and XGBoost (Random Search + CV) show very similar performance across all metrics. Random Forest achieves marginally lower RMSE, MAE, and MAPE, indicating slightly better prediction accuracy, while XGBoost attains a slightly higher R² and Adjusted R², suggesting comparable explanatory power. Overall, both models perform similarly, with Random Forest offering a small advantage in error reduction.

## 13. Conclusion

- Machine learning methodologies are effective for analysing large-scale public health datasets.
- Predictive models built using the BRFSS dataset successfully addressed:
  - Heart disease classification
  - BMI regression
- Behavioural, lifestyle, and socioeconomic factors were identified as significant predictors of:
  - Cardiovascular disease risk
  - Obesity-related outcomes
- Key modifiable lifestyle factors influencing health outcomes include:
  - Physical activity
  - Smoking behaviour
  - Dietary choices
  - Socioeconomic status
- Findings extend beyond descriptive analysis by providing empirical evidence to support:
  - Targeted prevention and intervention strategies
- Machine learning approaches demonstrate strong potential to enhance public health decision-making through:
